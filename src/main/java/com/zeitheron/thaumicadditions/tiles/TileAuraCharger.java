			...
			if(world.isRemote)
			{
				...
				for(int i = 0; i < rays; ++i)
				{
					...
					if(getRNG().nextInt(100) == 0)
					{
						...
						Vec3d vecc = new Vec3d(pos.getX() + .5F + cos * .3, pos.getY() + .5F, pos.getZ() + .5F + sin * .3);
						
						Vec3d vecd = veca.add(veca.subtract(vecc));
						
						ChainReaction ra = new ChainReaction(new ThunderRunnable(world, vecc, veca, getRNG().nextLong(), 10, 1F, new Layer(771, AURA.getColor(), true), new Layer(771, KnowledgeTAR.CAELES.getColor(), true)), 0);
						
						ChainReaction rb = new ChainReaction(new ThunderRunnable(world, veca, vecb, getRNG().nextLong(), 10, 1F, new Layer(771, AURA.getColor(), true), new Layer(771, KnowledgeTAR.CAELES.getColor(), true)), 7);
						ChainReaction rb_a = new ChainReaction(new VisSparkleRunnable(veca.x, veca.y, veca.z, vecd.x, vecd.y, vecd.z, KnowledgeTAR.CAELES.getColor()), 0);
						...
						ClientChainReactor.REACTOR.addChain(ra, rb, rb_a, rc, rc_a);
					}*/
				}
			}
		}

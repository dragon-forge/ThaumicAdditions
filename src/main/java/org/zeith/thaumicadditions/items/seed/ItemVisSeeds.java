		...
		implements IEssentiaContainerItem
{
	...
	public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected)
	{
		...
		if(a != null)
		{
			BlockVisCrop crop = BlocksTAR.VIS_CROPS.get(a);

			if(crop != null)
			{
				Field itemRaw = ReflectionUtil.getFieldByValue(ItemStack.class, stack, stack.getItem());
				Field delegate = ReflectionUtil.getFieldByValue(ItemStack.class, stack, stack.getItem().delegate);

				try
				{
					ReflectionUtil.setFinalField(itemRaw, stack, crop.seed);
					ReflectionUtil.setFinalField(delegate, stack, crop.seed.delegate);
				} catch(Throwable e)
				{
					e.printStackTrace();
				}
	...
	{
		if(stack.getItem() instanceof ItemVisSeedsNew)
			return ((ItemVisSeedsNew) stack.getItem()).getAspect();
	...
	{
		return Optional.ofNullable(BlocksTAR.VIS_CROPS.get(aspect))
				.map(c -> new ItemStack(c.seed))
				.orElseGet(() ->
				{

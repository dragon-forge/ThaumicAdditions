...
public class ResearchStageBuilder
{
	...
	public ResearchStageBuilder setRequiredCraft(ItemStack... items)
	{
		...
		if(entry.getCraft() != null && entry.getCraft().length > 0)
		{
			...
			for(int i = 0; i < n; ++i)
			{
				...
				int code = stack instanceof ItemStack ? ResearchManager.createItemStackHash((ItemStack) stack) : ("oredict:" + stack).hashCode();
	...
	{
		entry.setObtain(ArrayUtils.add(entry.getObtain(), oredict));
		return this;
	}
